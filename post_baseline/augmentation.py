...
This script collects images from a specified root directory, splits them into training and validation sets,
applies various augmentations to the training set, and saves the augmented images to an output directory.

The augmentations include (training set):
...
For the validation set, lighter augmentations are applied to avoid biasing validation results.

The script also allows for copying original images to the output directory without augmentation.

Note:
    The script assumes the input images are in JPG, JPEG or PNG format and are organized in subfolders by class.
...
# Val augmentation counts (tunable separately)
VAL_PER_ORIGINAL_COUNTS = {
    "non_defect": 199,
    "hole": 79,
    "lycra_cut": 79,
    "needln": 99,
...
def augment_val_image(image):
    """Light augmentations for validation images (to avoid biasing validation)."""
    if random.random() < PROBS["hflip"]:
        image = ImageOps.mirror(image)
    if random.random() < PROBS["vflip"]:
        image = ImageOps.flip(image)
...
def save_train_augmentation(images, cls, split, count_per_original):
    """Apply full augmentations for training data."""
    ...
    for img_path in tqdm(images, desc=f"Augmenting {split} - {cls}"):
        ...
        for _ in range(count_per_original):
            aug_img = augment_train_image(img)
            aug_img.save(os.path.join(save_dir, f"{cls}_{split}_{idx}.jpeg"))
            idx += 1


def save_val_augmentation(images, cls, split, count_per_original):
    """Apply lighter augmentations for validation data."""
    save_dir = os.path.join(OUTPUT_DIR, split, cls)
    os.makedirs(save_dir, exist_ok=True)
    idx = 0
    for img_path in tqdm(images, desc=f"Val Augmenting {split} - {cls}"):
        img = Image.open(img_path).convert("RGB")
        for _ in range(count_per_original):
            aug_img = augment_val_image(img)
            aug_img.save(os.path.join(save_dir, f"{cls}_{split}_{idx}.jpeg"))
            idx += 1
...
def main():
    ...
    for cls, images in train_split.items():
        ...
        save_train_augmentation(images, cls, "train", count_per_original)
        copy_originals(images, cls, "train")

    for cls, images in val_split.items():
        count_per_original = VAL_PER_ORIGINAL_COUNTS.get(cls, 0)
        save_val_augmentation(images, cls, "val", count_per_original)
